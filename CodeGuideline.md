...
- **src/providers/**: React context providers that hold shared state for more than one
  page. A provider file exports only the component; the context object and its types
  live in a sibling `.ts` file.
- **src/types/**: Shared domain types and interfaces (no runtime code).
- **src/pages/**: All route-level pages.  
  ...
  - Current pages: `home/` (route `/`) and `counter/` (route `/contador/:id`).
...
project-root/
  ...
  │   ├── lib/               # Utility functions and libraries
  │   ├── pages/             # Application pages (each page in its own subdirectory)
  │   ├── providers/         # React context providers (shared state)
...
## Contador Fácil: architecture notes

This app has **no backend, no authentication and no external API**. Everything runs
locally in the browser and all business logic is kept framework-agnostic so the same
rules can be reused in a future React Native/Expo build:

- `src/types/counter.ts`: `Counter` / `CounterEvent` domain types.
- `src/lib/counters.ts`: pure, immutable rules (create, increment, decrement, reset,
  rename, sort). No React, no DOM.
- `src/lib/storage.ts`: the only module that talks to `localStorage`, behind the
  `KeyValueStorage` interface (swap it for `AsyncStorage` on React Native).
- `src/lib/counter-repository.ts`: reads/writes the saved counters and sanitizes
  anything invalid.
- `src/lib/history.ts` / `src/lib/theme.ts`: history labels (pt-BR) and light/dark theme.
- `src/providers/counters-provider.tsx`: in-memory state shared by every screen,
  loaded from and persisted to local storage.
- `src/components/ad-slot.tsx`: reserved, no-op placement for a future Google AdMob
  banner (React Native/Expo). No ads library and no fake banners.

The interface is Portuguese only (pt-BR); UI strings live directly in the components
and the template i18n runtime is left untouched.
